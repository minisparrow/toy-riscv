//===-- TOYRISCVMCExpr.cpp - RISCV specific MC expression classes ------------===//
//
...
#define DEBUG_TYPE "toyriscvmcexpr"

const TOYRISCVMCExpr *TOYRISCVMCExpr::create(const MCExpr *Expr, VariantKind Kind,
                                       MCContext &Ctx) {
  return new (Ctx) TOYRISCVMCExpr(Expr, Kind);
}

void TOYRISCVMCExpr::printImpl(raw_ostream &OS, const MCAsmInfo *MAI) const {
...
const MCFixup *TOYRISCVMCExpr::getPCRelHiFixup(const MCFragment **DFOut) const {
  MCValue AUIPCLoc;
...
bool TOYRISCVMCExpr::evaluateAsRelocatableImpl(MCValue &Res,
...
void TOYRISCVMCExpr::visitUsedExpr(MCStreamer &Streamer) const {
  Streamer.visitUsedExpr(*getSubExpr());
}

TOYRISCVMCExpr::VariantKind TOYRISCVMCExpr::getVariantKindForName(StringRef name) {
  return StringSwitch<TOYRISCVMCExpr::VariantKind>(name)
      .Case("lo", VK_RISCV_LO)
      .Case("hi", VK_RISCV_HI)
...
StringRef TOYRISCVMCExpr::getVariantKindName(VariantKind Kind) {
  switch (Kind) {
...
void TOYRISCVMCExpr::fixELFSymbolsInTLSFixups(MCAssembler &Asm) const {
  switch (getKind()) {
  default:
    return;
  case VK_RISCV_TPREL_HI:
...
bool TOYRISCVMCExpr::evaluateAsConstant(int64_t &Res) const {
  MCValue Value;
...
int64_t TOYRISCVMCExpr::evaluateAsInt64(int64_t Value) const {
  switch (Kind) {
  default:
